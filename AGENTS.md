...
Use `npm run format` before the checks when files need formatting. Run Expo on
relevant devices for application changes. UI, gesture, accessibility,
...
Android tooling. State precisely when those environments are unavailable. Do not
run Expo prebuild or commit generated `ios/` or `android/` directories unless
the repository's managed/CNG policy is explicitly changed.
