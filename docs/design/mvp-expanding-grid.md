...
The fixed sRGB tile palette is a proposed presentation decision. It provides a
continuous cream → amber → terracotta → berry → violet → indigo progression;
colors supplement, but never replace, numeric values and accessible
descriptions.
...
exact values. The palette does not wrap, cap game values, or introduce a win or
game-over condition. Every listed foreground/background pair has at least 4.5:1
calculated contrast.
