...
The repository contains the MVP specification, quality tooling, and a minimal
Expo application shell. Install Node.js 24 with `nvm` and its bundled npm, clone
...
Then install and check the project:
...
Run `npm start` to launch the Expo development server. The current application
is a static shell; gameplay will be added in later scoped changes. After Xcode
is installed, `npm run ios` creates and runs a local development build through
Expo CNG. Do not commit generated `ios/` or `android/` directories. Expo is the
...
contributions. Report app, native, and device checks precisely, including any
