...
The presentation uses this fixed sRGB palette. These are explicit constants, not
colors interpolated at runtime. Color supplements the numeric value and
accessible description; it does not replace them.
...
the exponent-20 background and text colors while continuing to display their
exact values. The palette does not wrap or impose a value cap or game-over
condition. Every listed foreground/background pair has at least 4.5:1 contrast.
